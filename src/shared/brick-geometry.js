import { BoxBufferGeometry, Color, CylinderBufferGeometry, Matrix4, Quaternion, Vector3 } from 'three';
...
const GEOMETRY_STUD_SELECT_BOX = (function getStudBoxGeometry() {
  const stud = new BoxBufferGeometry(BRICK_WIDTH, STUD_HEIGHT, BRICK_WIDTH).toNonIndexed(); // We don't want to share any indices to simplify how we deal with triangles
  // Position...
...
const GEOMETRY_STUD_BOX = (function getStudBoxGeometry() {
  const stud = new BoxBufferGeometry(STUD_RADIUS * 2, STUD_HEIGHT, STUD_RADIUS * 2).toNonIndexed(); // We don't want to share any indices to simplify how we deal with triangles
  // Position...
  stud.translate(0, STUD_HEIGHT / 2, 0);
  return stud;
})();
...
function getBoxGeometryForBrickPart(width, depth, height) {
  ...
  geometry.translate(0, (height * BRICK_HEIGHT) / 2, 0);
  return geometry;
}
